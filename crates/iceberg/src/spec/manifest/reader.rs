...
impl ManifestReader {
    /// Create a reader for the manifest at `input_file`.
    ...
    pub async fn read(self, manifest_file: &ManifestFile) -> Result<Manifest> {
        ...
        let bytes = match key_metadata {
            Some(key_metadata) => {
                EncryptedInputFile::new(self.input_file, key_metadata)
                    .read()
                    .await?
            }
            None => self.input_file.read().await?,
        };
...
mod tests {
    ...
    fn test_entry(partition_spec_id: i32) -> ManifestEntry {
        ManifestEntry {
    ...
    async fn test_read_plaintext_manifest_inherits_entries() {
        ...
        .build_v2_data();
        writer.add_entry(test_entry(0)).unwrap();
        ...
        let manifest = ManifestReader::new(io.new_input(path).unwrap())
            .read(&manifest_file)
            .await
            .unwrap();
    ...
    async fn test_read_encrypted_manifest_roundtrip() {
        ...
        // Without the key metadata the encrypted bytes must not parse as plaintext.
        let mut plaintext_entry = manifest_file.clone();
        plaintext_entry.key_metadata = None;
        assert!(
            ManifestReader::new(io.new_input(path).unwrap())
